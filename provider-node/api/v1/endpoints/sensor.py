...
def get_sensor(request: Request):
    ...
    if sensor.status == OPMODE.DOWN or sensor.resource.charge <= 0:
...
def get_sensor_resource(request:Request, sensor_id: int):
    ...
    json_response= jsonable_encoder(sensores.get(sensors_names[sensor_id]).resource.get_resource_logs())
